...
# the autograd engine's backward thread — so we (re)register it at the top of both
# forward() and backward() instead of only at import time.
...
# LIGER_FUSED_MOE_MEMORY_EFFICIENT=1 drops the (TK, I) weighted_act buffer in
# backward: dW2 recomputes s_k*silu(gate)*up from pre_act on the fly. Saves
# ~TK*I*2 bytes of peak memory (e.g. 400 MB at T=32768, I=768, bf16) at ~5-10%
# backward slowdown from the redundant recompute across dW2's H-tiles.
_MEMORY_EFFICIENT = os.environ.get("LIGER_FUSED_MOE_MEMORY_EFFICIENT", "0").lower() in (
    "1",
    "true",
    "yes",
)
...
def _get_dW_autotune_configs():
    ...
    return [
        triton.Config(
            {"BLOCK_M": bm, "BLOCK_N": bn, "BLOCK_K": bk, "GROUP_M": 4},
            num_warps=nw,
            num_stages=ns,
        )
        for bm in [64, 128]
...
@triton.autotune(
    configs=_get_gemm_autotune_configs(),
    key=["H_dim", "I_dim", "BLOCK_M", "USE_TMA"],
    reset_to_zero=["out_ptr"],  # fused epilogue accumulates with atomics
)
@triton.jit
def _fused_down_proj_kernel(
    ...
    total_tiles_ptr,  # (1,) int32 — actual number of m-tiles (device scalar)
    x_gather_idx_ptr,  # (TK,) int32 — sorted_pos → token index
    s_scatter_idx_ptr,  # (TK,) int32 — sorted_pos → flat (t,k) index
    topk_weights_ptr,  # (TK,) — s_k in flat (t,k) order
    out_ptr,  # (T, H) fp32 — output accumulator (atomic epilogue, no Y buffer)
    ...
    Each CTA computes one (BLOCK_M, BLOCK_N) tile of post_act @ down_proj[e]^T and
    atomically accumulates s_k-weighted rows straight into out[token] (fp32) —
    the (TK, H) Y buffer and the separate token-gather kernel disappear."""
    pid = tl.program_id(0)
    ...
    # Routing metadata for the fused epilogue (constant across the K-loop).
    token_idx = tl.load(x_gather_idx_ptr + row_offs, mask=row_mask, other=0).to(tl.int64)
    ...
    # Fused epilogue: out[t] += s_k * y2 for each row (t, k) of this tile.
    acc = acc * weights[:, None]
    out_ptrs = out_ptr + token_idx[:, None] * stride_o_T + n_idx[None, :] * stride_o_H
    tl.atomic_add(out_ptrs, acc, mask=row_mask[:, None] & n_mask[None, :])
...
@triton.autotune(
    ...
    restore_value=["pre_act_ptr"],  # d_pre_act aliases pre_act (in-place); restore between tuning runs
)
@triton.jit
...
def _moe_bwd_dX_expanded_kernel(
    ...
    for k in tl.range(0, I_dim, BLOCK_K):
        ...
        if USE_TMA:
            w_gate = w_desc.load([w_row0 + k, n_start])
        else:
            w_gate_ptrs = (
        ...
        if USE_TMA:
            w_up = w_desc.load([w_row0 + I_dim + k, n_start])
        else:
            w_up_ptrs = (
                gate_up_proj_ptr
...
# Chosen adaptively per call from the average tokens-per-expert (host math, no sync).
BLOCK_M_TOKEN = 64
...
class LigerFusedMoEFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, gate_up_proj, down_proj, top_k_index, top_k_weights):
        ...
        # Fused aggregation: the down-proj kernel accumulates s_k-weighted expert
        # outputs directly into a zero-initialized fp32 buffer via atomics.
        # No (TK, H) Y buffer, no separate token-gather kernel.
        out_f32 = torch.zeros(T, H, dtype=torch.float32, device=x.device)
        ...
        if num_m_tiles > 0:
            _fused_down_proj_kernel[lambda meta: (num_m_tiles * triton.cdiv(H, meta["BLOCK_N"]),)](
                ...
                stride_o_T=out_f32.stride(0),
                stride_o_H=out_f32.stride(1),
                BLOCK_M=block_m_token,
                USE_TMA=use_tma_w2,
            )
    ...
    def backward(ctx, dO):
        ...
        # ---- dA' = dO @ W2^T, SwiGLU backward → d_pre_act (in-place), dS ----
        # d_pre_act aliases pre_act: each (row, n) element is consumed and produced
        # by the same CTA, so this is race-free and saves a (TK, 2I) buffer.
        d_pre_act = pre_act
        ...
        if num_m_tiles > 0:
            _moe_bwd_down_proj_kernel[lambda meta: (num_m_tiles * triton.cdiv(intermediate_dim, meta["BLOCK_N"]),)](
                ...
                stride_wact_TK=weighted_act.stride(0) if not _MEMORY_EFFICIENT else 0,
                stride_wact_I=weighted_act.stride(1) if not _MEMORY_EFFICIENT else 1,
                BLOCK_M=block_m_token,
                USE_TMA=use_tma_w2,
                WRITE_WACT=not _MEMORY_EFFICIENT,
            )
        # pre_act now holds d_pre_act; bump its autograd version so a second backward
        # through the same graph errors out instead of silently producing garbage.
        torch.autograd.graph.increment_version(pre_act)

        if not _MEMORY_EFFICIENT:
            _moe_bwd_dW2_kernel[
                lambda meta: (E * triton.cdiv(intermediate_dim, meta["BLOCK_M"]) * triton.cdiv(H, meta["BLOCK_N"]),)
            ](
                weighted_act,
