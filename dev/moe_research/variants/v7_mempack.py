...
def _moe_bwd_down_proj_kernel(
    ...
    dS_ptr,  # (TK,)    — output (fp32): ∂L/∂s_k, indexed by flat (t,k)
    H_dim: tl.constexpr,
    I_dim: tl.constexpr,
    stride_dO_T,
    ...
    each (row, n) element is read and written by exactly the same CTA) and dS.
    weighted_act is NOT materialized; dW2 recomputes s_k*y1 from pre_act."""
    pid = tl.program_id(0)
    NUM_PID_N: tl.constexpr = (I_dim + BLOCK_N - 1) // BLOCK_N
    num_pid_m = tl.load(total_tiles_ptr)
    if pid >= num_pid_m * NUM_PID_N:
    ...
    # dS buffer is fp32: bf16 atomics would round each of the ceil(I/BLOCK_N)
    # partial contributions and lose precision.
...
def _moe_bwd_dW2_kernel(
    pre_act_ptr,  # (TK, 2I) — z = [gate, up] saved from forward (y1 recomputed here)
    dout_ptr,  # (T, H)  — upstream gradient (gathered by x_gather_idx)
    x_gather_idx_ptr,  # (TK,)   — sorted_pos → original token index
    s_scatter_idx_ptr,  # (TK,)  — sorted_pos → flat (t,k) index (for routing weights)
    ...
    with s_k*y1 recomputed on the fly from pre_act (saves materializing the
    (TK, I) weighted_act buffer: one less alloc, one less store+load round trip).
    MUST run BEFORE _moe_bwd_down_proj_kernel, which overwrites pre_act in place.
    Grid: 1D (E * ceil(I/BLOCK_M) * ceil(H/BLOCK_N),), expert-major so all tiles of
    one expert are resident together: its pre_act/dO rows stay in L2 instead of
    ...
    for k in tl.range(0, M_e, BLOCK_K):
        ...
        # Cast to the activation dtype so numerics match the materialized-buffer path.
...
class LigerFusedMoEFunction(torch.autograd.Function):
    ...
    def backward(ctx, dO):
        ...
        # dW2 = (s_k * y1)^T @ dO_gathered — recomputes s_k*y1 from pre_act, so it
        # MUST run before the bwd-down-proj kernel overwrites pre_act in place.
        ...
        # d_pre_act aliases pre_act (in-place): each (row, n) element is consumed and
        # produced by the same CTA, so this is race-free and saves a (TK, 2I) buffer.
        d_pre_act = pre_act
        # fp32 dS: atomic_add accumulates ceil(I/BLOCK_N) partials per element;
        # bf16 atomics would round every partial.
        ...
        # pre_act's contents were replaced by d_pre_act; bump its autograd version so
        # a second backward through the same graph errors out instead of silently
        # producing garbage.
