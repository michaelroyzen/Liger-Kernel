"""
Variant v15_fixdx — B300/sm103 correctness fix over the shipped src kernel.

Base: src/liger_kernel/ops/fused_moe{,_kernels}.py @ aeab3e3 (the H100-optimized final).
Change: _moe_bwd_dX_expanded_kernel computes dx_expanded = d_pre_act @ W1[e] as a
SINGLE GEMM over K=2I instead of two half-GEMMs (gate/up) chained through one
accumulator. Triton 3.7.1 miscompiles the two-dots-one-accumulator pattern on
sm103 (tcgen05 MMA path) for most tile configs — see probe_sm103_dot.py.
The merged form is mathematically identical (d_pre_act columns and W1 rows share
the same [gate; up] ordering) and architecture-neutral.
"""
