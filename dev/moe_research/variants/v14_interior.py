...
def _fused_up_proj_swiglu_kernel(
    ...
    n_mask = n_idx < I_dim
    # Interior tiles (all rows valid) skip the row predicate on the hot x load:
    # fewer predicate registers and cleaner cp.async vectorization in the K-loop.
    full_tile = row_start + BLOCK_M <= expert_end
    if USE_TMA:
    ...
    if USE_TMA and full_tile:
        for k in tl.range(0, H_dim, BLOCK_K):
            x_ptrs = x_ptr + token_idx[:, None] * stride_x_T + (k + k_offs)[None, :] * stride_x_H
            x_tile = tl.load(x_ptrs, eviction_policy="evict_first")
