...
def _fused_up_proj_swiglu_kernel(
    ...
    if pid < num_pid_m * NUM_PID_N:
        ...
        for k in tl.range(0, H_dim, BLOCK_K, warp_specialize=True):
            k_idx = k + k_offs
            k_mask = k_idx < H_dim
...
def _fused_down_proj_kernel(
    ...
    if pid < num_pid_m * NUM_PID_N:
        ...
        for k in tl.range(0, I_dim, BLOCK_K, warp_specialize=True):
            k_idx = k + k_offs
            k_mask = k_idx < I_dim
            ...
            if USE_TMA:
                ...
                w_ptrs = (
                    down_proj_ptr + expert_idx * stride_w_E + n_idx[:, None] * stride_w_H + k_idx[None, :] * stride_w_I
                )
                w_tile = tl.load(
                    w_ptrs,
    ...
    if pid < num_pid_m * NUM_PID_N:
        ...
        for k in tl.range(0, H_dim, BLOCK_K, warp_specialize=True):
            ...
            if USE_TMA:
                ...
                w_ptrs = (
                    down_proj_ptr + expert_idx * stride_w_E + k_idx[:, None] * stride_w_H + n_idx[None, :] * stride_w_I
                )
