"""
Variant v18_dwkey — per-regime autotune keys for the dW kernels.
Based on: v16_bwtune (Blackwell config spaces) on v15_fixdx (merged-K dX bugfix).

Change: _moe_bwd_dW1_kernel/_moe_bwd_dW2_kernel currently key their autotune
cache on (H_dim, I_dim) only — the config picked at whatever token count runs
first is silently reused for every other token count. But the right dW tile is
regime-dependent: at ~64 tokens/expert the kernels are output-write-bound (the
K-loop is one iteration; small tiles = more parallelism), at ~4096 tokens/expert
they are K-loop-bound (deep-K tiles amortize the output write). The grouped
GEMM kernels already carry a regime signal in their key (BLOCK_M, picked from
tokens/expert by the host); the dW kernels carry none. Add TPE_BUCKET — a
host-computed power-of-2 bucket of TK//E — as an unused constexpr autotune key,
mirroring E5's "add BLOCK_M to the GEMM keys" which was worth 1.28x on H100.
Architecture-neutral (the key only splits the cache; spaces unchanged).
"""
...
def _moe_bwd_dX_expanded_kernel(
    ...
    # two-accumulator loops are unaffected; see dev/moe_research/probe_sm103_dot.py).
